...
public class WalksController : ControllerBase
{
    ...
    // init filtering
    [HttpGet]
    public async Task<IActionResult> getAllWalks([FromQuery] string? filterOn, [FromQuery] string? filterQuery, [FromQuery] string? sortBy, [FromQuery] string? sortOrder)
    {
        // getting them from repo
        var walksDomain = await walkRepository.GetAllAsync(filterOn, filterQuery, sortBy, sortOrder);
